...
__global__ void InputBatch_8_Input_7x7_InChannel_672_OutChannel_192(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 21 * 32928 + (blockIdx.x % 21) / 3 * 7 + (blockIdx.x % 3) / 3 * 7;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 3) * 43008;
