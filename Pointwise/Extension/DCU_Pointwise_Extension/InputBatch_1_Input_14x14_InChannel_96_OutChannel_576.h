...
InputBatch_1_Input_14x14_InChannel_96_OutChannel_576

Grid:
    gridDim.x = (1 * 576 * 14 * 14) / (4 * 7 * 32);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_1_Input_14x14_InChannel_96_OutChannel_576(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 126 * 18816 + (blockIdx.x % 126) / 18 * 28 + (blockIdx.x % 18) / 18 * 14;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 18) * 3072;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 126 * 112896 + (blockIdx.x % 126) / 18 * 28 + (blockIdx.x % 18) / 18 * 14 + (blockIdx.x % 18) * 6272;
