...
InputBatch_32_Input_56x56_InChannel_48_OutChannel_24

Grid:
    gridDim.x = (32 * 24 * 56 * 56) / (4 * 7 * 24);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_32_Input_56x56_InChannel_48_OutChannel_24(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 112 * 150528 + (blockIdx.x % 112) / 8 * 224 + (blockIdx.x % 8) / 1 * 7;
