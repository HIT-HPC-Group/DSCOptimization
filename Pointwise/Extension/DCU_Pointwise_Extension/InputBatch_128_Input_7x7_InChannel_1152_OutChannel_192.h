...
InputBatch_128_Input_7x7_InChannel_1152_OutChannel_192
...
__global__ void InputBatch_128_Input_7x7_InChannel_1152_OutChannel_192(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    filterSharedBuffer1[threadIdx.x + 5 * 256] = filter[blockLoadFilterStartIdx + ((threadIdx.x + 5 * 256) / 16) * 1152 + ((threadIdx.x + 5 * 256) % 16)];
    __syncthreads();
    ...
    for(int i = 0; i < (inputChannel - 16) / (2 * 16); i++) {
        ...
        filterTemp6 = filter[blockLoadFilterStartIdx + ((threadIdx.x + 5 * 256) / 16) * 1152 + ((threadIdx.x + 5 * 256) % 16)];
