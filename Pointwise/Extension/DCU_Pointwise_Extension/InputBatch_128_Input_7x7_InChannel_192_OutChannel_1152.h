...
InputBatch_128_Input_7x7_InChannel_192_OutChannel_1152

Grid:
    gridDim.x = (128 * 1152 * 7 * 7) / (7 * 7 * 48);
Block:
    blockDim.x = 64 * 7;
...
__global__ __launch_bounds__(1024) void InputBatch_128_Input_7x7_InChannel_192_OutChannel_1152(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 24 * 9408 + (blockIdx.x % 24) / 24 * 49 + (blockIdx.x % 24) / 24 * 7;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 24) * 9216;
    if(threadIdx.x < 1 * 48 * 4 - 0 * 448) {
        filterSharedBuffer1[threadIdx.x + 0 * 448] = filter[blockLoadFilterStartIdx + ((threadIdx.x + 0 * 448) / 4) * 192 + ((threadIdx.x + 0 * 448) % 4)];
    }
    __syncthreads();
    ...
    for(int i = 0; i < (inputChannel - 4) / (2 * 4); i++) {
        ...
        if(threadIdx.x < 1 * 48 * 4 - 0 * 448) {
            filterTemp1 = filter[blockLoadFilterStartIdx + ((threadIdx.x + 0 * 448) / 4) * 192 + ((threadIdx.x + 0 * 448) % 4)];
        }
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 24 * 56448 + (blockIdx.x % 24) / 24 * 49 + (blockIdx.x % 24) / 24 * 7 + (blockIdx.x % 24) * 2352;
