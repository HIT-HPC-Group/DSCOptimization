...
InputBatch_8_Input_14x14_InChannel_192_OutChannel_64

Grid:
    gridDim.x = (8 * 64 * 14 * 14) / (4 * 14 * 16);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_8_Input_14x14_InChannel_192_OutChannel_64(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 14 * 37632 + (blockIdx.x % 14) / 2 * 28 + (blockIdx.x % 2) / 2 * 14;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 2) * 6144;
