...
__global__ void InputBatch_1_Input_7x7_InChannel_672_OutChannel_192(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 84 * 32928 + (blockIdx.x % 84) / 12 * 7 + (blockIdx.x % 12) / 12 * 7;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 12) * 10752;
