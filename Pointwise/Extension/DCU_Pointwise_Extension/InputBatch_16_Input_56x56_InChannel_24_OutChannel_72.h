...
InputBatch_16_Input_56x56_InChannel_24_OutChannel_72

Grid:
    gridDim.x = (16 * 72 * 56 * 56) / (4 * 7 * 24);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_16_Input_56x56_InChannel_24_OutChannel_72(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 336 * 75264 + (blockIdx.x % 336) / 24 * 224 + (blockIdx.x % 24) / 3 * 7;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 336 * 225792 + (blockIdx.x % 336) / 24 * 224 + (blockIdx.x % 24) / 3 * 7 + (blockIdx.x % 3) * 75264;
