...
InputBatch_16_Input_28x28_InChannel_120_OutChannel_40

Grid:
    gridDim.x = (16 * 40 * 28 * 28) / (4 * 7 * 40);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_16_Input_28x28_InChannel_120_OutChannel_40(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 28 * 94080 + (blockIdx.x % 28) / 4 * 112 + (blockIdx.x % 4) / 1 * 7;
