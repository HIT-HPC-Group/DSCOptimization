...
InputBatch_1_Input_14x14_InChannel_576_OutChannel_96
...
__global__ void InputBatch_1_Input_14x14_InChannel_576_OutChannel_96(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 168 * 112896 + (blockIdx.x % 168) / 12 * 14 + (blockIdx.x % 12) / 6 * 7;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 168 * 18816 + (blockIdx.x % 168) / 12 * 14 + (blockIdx.x % 12) / 6 * 7 + (blockIdx.x % 6) * 3136;
