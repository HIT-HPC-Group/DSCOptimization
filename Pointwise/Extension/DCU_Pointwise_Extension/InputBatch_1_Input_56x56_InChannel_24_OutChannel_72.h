...
InputBatch_1_Input_56x56_InChannel_24_OutChannel_72

Grid:
    gridDim.x = (1 * 72 * 56 * 56) / (4 * 4 * 24);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_1_Input_56x56_InChannel_24_OutChannel_72(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 588 * 75264 + (blockIdx.x % 588) / 42 * 224 + (blockIdx.x % 42) / 3 * 4;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 3) * 576;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 588 * 225792 + (blockIdx.x % 588) / 42 * 224 + (blockIdx.x % 42) / 3 * 4 + (blockIdx.x % 3) * 75264;
