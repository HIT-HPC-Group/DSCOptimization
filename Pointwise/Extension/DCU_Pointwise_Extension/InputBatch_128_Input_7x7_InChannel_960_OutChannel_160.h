...
InputBatch_128_Input_7x7_InChannel_960_OutChannel_160

Grid:
    gridDim.x = (128 * 160 * 7 * 7) / (4 * 7 * 40);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_128_Input_7x7_InChannel_960_OutChannel_160(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 7 * 47040 + (blockIdx.x % 7) / 1 * 7 + (blockIdx.x % 1) / 1 * 7;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 1) * 153600;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 7 * 7840 + (blockIdx.x % 7) / 1 * 7 + (blockIdx.x % 1) / 1 * 7 + (blockIdx.x % 1) * 7840;
