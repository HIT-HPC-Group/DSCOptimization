...
InputBatch_32_Input_7x7_InChannel_192_OutChannel_1152

Grid:
    gridDim.x = (32 * 1152 * 7 * 7) / (7 * 7 * 96);
Block:
    blockDim.x = 64 * 7;
...
__global__ __launch_bounds__(1024) void InputBatch_32_Input_7x7_InChannel_192_OutChannel_1152(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 12 * 9408 + (blockIdx.x % 12) / 12 * 49 + (blockIdx.x % 12) / 12 * 7;
    ...
    if(threadIdx.x < 1 * 96 * 2 - 0 * 448) {
        filterSharedBuffer1[threadIdx.x + 0 * 448] = filter[blockLoadFilterStartIdx + ((threadIdx.x + 0 * 448) / 2) * 192 + ((threadIdx.x + 0 * 448) % 2)];
    }
    __syncthreads();
    ...
    for(int i = 0; i < (inputChannel - 2) / (2 * 2); i++) {
        ...
        if(threadIdx.x < 1 * 96 * 2 - 0 * 448) {
            filterTemp1 = filter[blockLoadFilterStartIdx + ((threadIdx.x + 0 * 448) / 2) * 192 + ((threadIdx.x + 0 * 448) % 2)];
        }
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 12 * 56448 + (blockIdx.x % 12) / 12 * 49 + (blockIdx.x % 12) / 12 * 7 + (blockIdx.x % 12) * 4704;
