...
InputBatch_32_Input_28x28_InChannel_40_OutChannel_120

Grid:
    gridDim.x = (32 * 120 * 28 * 28) / (4 * 4 * 40);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_32_Input_28x28_InChannel_40_OutChannel_120(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 147 * 31360 + (blockIdx.x % 147) / 21 * 112 + (blockIdx.x % 21) / 3 * 4;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 147 * 94080 + (blockIdx.x % 147) / 21 * 112 + (blockIdx.x % 21) / 3 * 4 + (blockIdx.x % 3) * 31360;
