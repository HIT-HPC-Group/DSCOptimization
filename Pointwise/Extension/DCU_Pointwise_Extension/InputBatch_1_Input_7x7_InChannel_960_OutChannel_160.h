...
__global__ void InputBatch_1_Input_7x7_InChannel_960_OutChannel_160(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 70 * 47040 + (blockIdx.x % 70) / 10 * 7 + (blockIdx.x % 10) / 10 * 7;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 70 * 7840 + (blockIdx.x % 70) / 10 * 7 + (blockIdx.x % 10) / 10 * 7 + (blockIdx.x % 10) * 784;
