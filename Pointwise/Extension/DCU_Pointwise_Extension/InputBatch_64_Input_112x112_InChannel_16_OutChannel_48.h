...
InputBatch_64_Input_112x112_InChannel_16_OutChannel_48

Grid:
    gridDim.x = (64 * 48 * 112 * 112) / (4 * 4 * 48);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_64_Input_112x112_InChannel_16_OutChannel_48(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 784 * 200704 + (blockIdx.x % 784) / 28 * 448 + (blockIdx.x % 28) / 1 * 4;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 1) * 768;
    ...
    // Store output
    int blockWriteOutputStartIdx = blockIdx.x / 784 * 602112 + (blockIdx.x % 784) / 28 * 448 + (blockIdx.x % 28) / 1 * 4 + (blockIdx.x % 1) * 602112;
