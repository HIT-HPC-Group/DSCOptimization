...
InputBatch_8_Input_14x14_InChannel_384_OutChannel_96
...
__global__ void InputBatch_8_Input_14x14_InChannel_384_OutChannel_96(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 21 * 75264 + (blockIdx.x % 21) / 3 * 28 + (blockIdx.x % 3) / 3 * 14;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 3) * 12288;
    ...
    filterSharedBuffer1[threadIdx.x + 2 * 256] = filter[blockLoadFilterStartIdx + ((threadIdx.x + 2 * 256) / 32) * 384 + ((threadIdx.x + 2 * 256) % 32)];
    filterSharedBuffer1[threadIdx.x + 3 * 256] = filter[blockLoadFilterStartIdx + ((threadIdx.x + 3 * 256) / 32) * 384 + ((threadIdx.x + 3 * 256) % 32)];
    __syncthreads();
    ...
    for(int i = 0; i < (inputChannel - 32) / (2 * 32); i++) {
        ...
        filterTemp3 = filter[blockLoadFilterStartIdx + ((threadIdx.x + 2 * 256) / 32) * 384 + ((threadIdx.x + 2 * 256) % 32)];
        filterTemp4 = filter[blockLoadFilterStartIdx + ((threadIdx.x + 3 * 256) / 32) * 384 + ((threadIdx.x + 3 * 256) % 32)];
