...
InputBatch_64_Input_28x28_InChannel_72_OutChannel_40

Grid:
    gridDim.x = (64 * 40 * 28 * 28) / (4 * 7 * 40);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_64_Input_28x28_InChannel_72_OutChannel_40(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 28 * 56448 + (blockIdx.x % 28) / 4 * 112 + (blockIdx.x % 4) / 1 * 7;
    ...
    // filter
    int blockLoadFilterStartIdx = (blockIdx.x % 1) * 2880;
    ...
    if(threadIdx.x < 1 * 40 * 8 - 1 * 256) {
        filterSharedBuffer1[threadIdx.x + 1 * 256] = filter[blockLoadFilterStartIdx + ((threadIdx.x + 1 * 256) / 8) * 72 + ((threadIdx.x + 1 * 256) % 8)];
    }
    __syncthreads();
    ...
    for(int i = 0; i < (inputChannel - 8) / (2 * 8); i++) {
        ...
        if(threadIdx.x < 1 * 40 * 8 - 1 * 256) {
            filterTemp2 = filter[blockLoadFilterStartIdx + ((threadIdx.x + 1 * 256) / 8) * 72 + ((threadIdx.x + 1 * 256) % 8)];
        }
