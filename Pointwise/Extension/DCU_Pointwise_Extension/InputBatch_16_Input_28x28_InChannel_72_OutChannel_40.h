...
InputBatch_16_Input_28x28_InChannel_72_OutChannel_40

Grid:
    gridDim.x = (16 * 40 * 28 * 28) / (4 * 4 * 40);
Block:
    blockDim.x = 64 * 4;
...
__global__ void InputBatch_16_Input_28x28_InChannel_72_OutChannel_40(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    // input
    int blockLoadInputStartIdx = blockIdx.x / 49 * 56448 + (blockIdx.x % 49) / 7 * 112 + (blockIdx.x % 7) / 1 * 4;
