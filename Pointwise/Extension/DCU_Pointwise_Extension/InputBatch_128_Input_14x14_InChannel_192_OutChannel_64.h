...
InputBatch_128_Input_14x14_InChannel_192_OutChannel_64
...
__global__ void InputBatch_128_Input_14x14_InChannel_192_OutChannel_64(const scalar_t* __restrict__ input, const scalar_t* __restrict__ filter, scalar_t* __restrict__ output,
    ...
    filterSharedBuffer1[threadIdx.x + 0 * 256] = filter[blockLoadFilterStartIdx + ((threadIdx.x + 0 * 256) / 4) * 192 + ((threadIdx.x + 0 * 256) % 4)];
    __syncthreads();
    ...
    for(int i = 0; i < (inputChannel - 4) / (2 * 4); i++) {
        ...
        filterTemp1 = filter[blockLoadFilterStartIdx + ((threadIdx.x + 0 * 256) / 4) * 192 + ((threadIdx.x + 0 * 256) % 4)];
