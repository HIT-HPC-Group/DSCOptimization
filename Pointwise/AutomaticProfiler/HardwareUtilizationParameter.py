...
# Max active warp per SM
GPU_MAX_ACTIVE_WARP = 32
DCU_MAX_ACTIVE_WARP = 40
...
def calculateUtilizationandAI(device, blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, outputBatchNumber, outputChannel, outputHeight, outputWidth, horizontalRepeat, verticalRepeat):
    ...
    totalRegisterPerThread = resultRegisterNumPerThread + operandRegisterNumPerThread + tempRegisterNumPerThread + 30 # not sure if 40 is accurate
    ...
    if totalRegisterPerThread > registerLimitPerThread or totalSharedMemPerBlock > sharedMemLimitPerBlock:
        return None
    ...
    if device == "GPU":
        ...
        SMUtilization = totalBlockNum / (blockNumPerSM * DCU_TOTAL_SM_NUM)

    ArithmeticIntensity = outputWidthPerWarp * outputChannelPerThread / (outputWidthPerWarp + outputChannelPerThread)

    if device == "GPU":
        SMOccupancy = min(blockNumPerSM * warpNumPerBlock, 64) / 64
    elif device == "DCU":
        SMOccupancy = min(blockNumPerSM * warpNumPerBlock, 40) / 40

    return [SMUtilization, ArithmeticIntensity, SMOccupancy]
...
def getCombinationOptions(device, batchSize, layerConfig):
    ...
    blockNumPerSMOptions = list(range(1, 101))
    # input 112 x 112
    # checked
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 16 and inputWidth == 112 and outputChannel == 48:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 8, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 48 and inputWidth == 56 and outputChannel == 24:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 8, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 24 and inputWidth == 56 and outputChannel == 72:
        for blockNumPerSM in blockNumPerSMOptions:
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 72 and inputWidth == 56 and outputChannel == 24:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        if repeat[1] == 1:
                            outputChannelPerWarpOptions = [8, 24]
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            if device == "GPU":
                                if outputChannelPerWarp == 8:
                                    channelGroupSizeOptions = [8, 4]
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 72 and inputWidth == 28 and outputChannel == 40:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        if repeat[1] == 1:
                            outputChannelPerWarpOptions = [8, 40]
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            if device == "GPU":
                                if outputChannelPerWarp == 8:
                                    channelGroupSizeOptions = [8, 4]
    ...
    elif inputChannel == 24 and inputWidth == 28 and outputChannel == 24:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
    ...
    elif inputChannel == 40 and inputWidth == 28 and outputChannel == 120:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 120 and inputWidth == 28 and outputChannel == 40:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
        ...
        for blockNumPerSM in blockNumPerSMOptions:
            ...
            for warpNumPerBlock in warpNumPerBlockOptions:
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        if repeat[1] == 1:
                            outputChannelPerWarpOptions = [2, 4, 8, 12, 16, 24, 32, 48, 64, 96, 128, 192]
                        elif repeat[1] == 2:
                            outputChannelPerWarpOptions = [2, 4, 8, 12, 16, 24, 32, 48, 64, 96]
                        elif repeat[1] == 4:
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 480 and inputWidth == 14 and outputChannel == 96:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 8, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 48 and inputWidth == 14 and outputChannel == 48:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 8, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        if repeat[1] == 1:
                            ...
                            outputChannelPerWarpOptions = [2, 10]
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            if device == "GPU":
                                if outputChannelPerWarp == 2:
                                    channelGroupSizeOptions = [32, 16]
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        if repeat[1] == 1:
                            ...
                            outputChannelPerWarpOptions = [2, 4, 6, 8, 10, 12, 16, 20, 24, 30, 40, 48, 60, 80, 120, 240]
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            if device == "GPU":
                                if outputChannelPerWarp == 2:
                                    channelGroupSizeOptions = [32, 16]
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 96 and inputWidth == 7 and outputChannel == 96:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        if repeat[1] == 1:
                            outputChannelPerWarpOptions = [2, 4, 6, 8, 12, 16, 24, 32, 48, 96]
                        elif repeat[1] == 4:
                            outputChannelPerWarpOptions = [2, 4, 6, 8, 12, 24]
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            if device == "GPU":
                                if outputChannelPerWarp == 2:
                                    channelGroupSizeOptions = [32, 16]
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 576 and inputWidth == 7 and outputChannel == 192:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        ...
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            ...
                            for channelGroupSize in channelGroupSizeOptions:
                                combinationOptions.append([blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, repeat[0], repeat[1]])
    elif inputChannel == 192 and inputWidth == 7 and outputChannel == 1024:
        for blockNumPerSM in blockNumPerSMOptions:
            warpNumPerBlockOptions = [4, 7, 14]
            for warpNumPerBlock in warpNumPerBlockOptions:
                # horizontal repeat, vertical repeat
                ...
                for repeat in repeatOptions:
                    ...
                    for outputWidthPerWarp in outputWidthPerWarpOptions:
                        if repeat[1] == 1:
                            outputChannelPerWarpOptions = [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
                        elif repeat[1] == 2:
                            outputChannelPerWarpOptions = [2, 4, 8, 16, 32, 64, 128, 256, 512]
                        elif repeat[1] == 4:
                            outputChannelPerWarpOptions = [2, 4, 8, 16, 32, 64, 128, 256]
                        for outputChannelPerWarp in outputChannelPerWarpOptions:
                            if device == "GPU":
                                if outputChannelPerWarp == 2:
                                    channelGroupSizeOptions = [32, 16]
                                    ...
                                    channelGroupSizeOptions = [32, 16, 8, 4, 2, 1]
                                elif outputChannelPerWarp == 512:
                                    channelGroupSizeOptions = [32, 16, 8, 4, 2, 1]
                                elif outputChannelPerWarp == 1024:
                                    channelGroupSizeOptions = [32, 16, 8, 4, 2, 1]
                            else:
                                if outputChannelPerWarp == 2:
                                    channelGroupSizeOptions = [64, 32]
...
def getBestHardwareResourceParameters(device, batchSize, layerConfig):
    ...
    for combination in combinationOptions:
        ...
        if utilizationAI != None:
            validCombinationUtilizationAI.append([device, combination[0], combination[1], combination[2], combination[3], combination[4], batchSize, layerConfig[2], layerConfig[1], layerConfig[1], combination[5], combination[6], utilizationAI[0], utilizationAI[1], utilizationAI[2]])
...
def getHardwareResourceParametersList(device, batchSize, layerConfig):
    # 1. For each layer config, get all possible hardware resource parameters
    # list of [blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, horizontalRepeat, verticalRepeat]
    combinationOptions = getCombinationOptions(device, batchSize, layerConfig)
    # 2. From all possible parameter combinations (include invalid ones) , find those valid ones and 
    ...
    if len(utilizationSmallerThan1) != 0:
        sortedUtilizationSmallerThan1 = sorted(utilizationSmallerThan1, key = lambda x: x[12], reverse = True) # smutilization large to small
        large = sortedUtilizationSmallerThan1[0]
        result.append(large)
        for i in range(1, len(sortedUtilizationSmallerThan1)):
            if sortedUtilizationSmallerThan1[i][13] >= large[13]:
                result.append(sortedUtilizationSmallerThan1[i])

    if len(utilizationGreterThan1) != 0:
        sortedUtilizationGreterThan1 = sorted(utilizationGreterThan1, key = lambda x: x[12], reverse = False) # smutilization small to large
        small = sortedUtilizationGreterThan1[0]
        result.append(small)
        for i in range(1, len(sortedUtilizationGreterThan1)):
            if sortedUtilizationGreterThan1[i][13] >= small[13]:
                result.append(sortedUtilizationGreterThan1[i])

    return result
"""

"""
# Find from the "worse" ones
def getHardwareResourceParametersList2(device, batchSize, layerConfig):
    # 1. For each layer config, get all possible hardware resource parameters
    # list of [blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, horizontalRepeat, verticalRepeat]
    combinationOptions = getCombinationOptions(device, batchSize, layerConfig)
    # 2. From all possible parameter combinations (include invalid ones) , find those valid ones and 
    ...
    if len(utilizationSmallerThan1) != 0:
        sortedUtilizationSmallerThan1 = sorted(utilizationSmallerThan1, key = lambda x: x[12], reverse = True) # smutilization large to small
        large = sortedUtilizationSmallerThan1[0]
        toRemove.append(large)
        for i in range(1, len(sortedUtilizationSmallerThan1)):
            if sortedUtilizationSmallerThan1[i][13] >= large[13]:
                toRemove.append(sortedUtilizationSmallerThan1[i])

    if len(utilizationGreterThan1) != 0:
        sortedUtilizationGreterThan1 = sorted(utilizationGreterThan1, key = lambda x: x[12], reverse = False) # smutilization small to large
        small = sortedUtilizationGreterThan1[0]
        toRemove.append(small)
        for i in range(1, len(sortedUtilizationGreterThan1)):
            if sortedUtilizationGreterThan1[i][13] >= small[13]:
                toRemove.append(sortedUtilizationGreterThan1[i])

    result = [item for item in validCombinationUtilizationAI if item not in toRemove]
    return result
"""


# Simply Test all possible parameters.
def getAllHardwareResourceParameters(device, batchSize, layerConfig):
    # 1. For each layer config, get all possible hardware resource parameters
    # list of [blockNumPerSM, warpNumPerBlock, outputChannelPerWarp, outputWidthPerWarp, channelGroupSize, horizontalRepeat, verticalRepeat]
    combinationOptions = getCombinationOptions(device, batchSize, layerConfig)
    # 2. From all possible parameter combinations (include invalid ones) , find those valid ones and 
    ...
    return validCombinationUtilizationAI


def main():
    # batch size
    batchSizeOptions = [1, 8, 16, 32, 64]
    ...
    with open("count.txt", "w") as f:
        ...
        for layerConfig in layerConfigs:
            ...
            for batchSize in batchSizeOptions:
                s = 0
                bestParameters = getAllHardwareResourceParameters("DCU", batchSize, layerConfig)
                tmpList = []
                for i in bestParameters:
                    tmp = '{} {} {} {} {} {} {} {} {} {}'.format(int(layerConfig[0]), int(layerConfig[1]), int(layerConfig[2]), int(batchSize), int(i[2]), int(i[3]), int(i[4]), int(i[5]), int(i[10]), int(i[11]))

                    finalResult = finalResult + tmp + '\n'
                    s += 1
                finalResult += "=================\n"
                f.write(str(layerConfig) + ' ' + str(batchSize) + ' ' + str(s) + '\n')
